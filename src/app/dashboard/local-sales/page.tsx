...
export default function LocalSalesPage() {
  ...
  async function handleAddSale(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!clientId || !quantity || !price || !status || !invoiceNumber || !productId) {
    ...
    try {
      ...
      notifyChanged('local_sales', 'products');
  ...
  const handleEditStatus = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const formData = new FormData(event.currentTarget);
    const newStatus = formData.get('status') as SaleStatus | null;
    const submittedPaymentStatus = formData.get('paymentStatus') as PaymentStatus | null;
    const newPaymentStatus: PaymentStatus = submittedPaymentStatus ?? selectedSale.paymentStatus;

    if (!newStatus) {
        toast({ variant: 'destructive', title: 'Validation Error', description: 'Please select both statuses.' });
        return;
    }
    if (selectedSale.paymentStatus === 'Paid' && newPaymentStatus !== 'Paid') {
    ...
    try {
      ...
      notifyChanged('local_sales');
      if (newPaymentStatus === 'Paid' && selectedSale.paymentStatus !== 'Paid') {
          ...
          <form onSubmit={handleEditStatus}>
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Select name="paymentStatus" defaultValue={selectedSale?.paymentStatus} required disabled={selectedSale?.paymentStatus === 'Paid'}>
                  <SelectTrigger id="edit-payment-status">
                    <SelectValue placeholder="Select payment status" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="Pending" disabled={selectedSale?.paymentStatus === 'Paid'}>Pending</SelectItem>
