...
export default function FinancePage() {
  ...
  async function handleAddEntry(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (quantity && (price === undefined || Number.isNaN(price))) {
      toast({ variant: 'destructive', title: 'Validation Error', description: 'Price is required when quantity is provided.' });
      return;
    }
  ...
  const handleGeneratePdf = () => {
    ...
    if(uniqueTransactions.length > 0) {
        const tableData = uniqueTransactions.map(t => [
            ...
            t.category,
            t.clientName || 'N/A',
            t.quantity?.toLocaleString() || 'N/A',
            t.price !== undefined ? `$${t.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : 'N/A',
        ...
        autoTable(doc, {
            startY: finalY,
            head: [['Date', 'Description', 'Category/Product', 'Client', 'Qty', 'Price', 'Type', 'Amount']],
            body: tableData,
            theme: 'grid',
            styles: { fontSize: 8 },
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <form onSubmit={handleAddEntry}>
              <div className="space-y-4 py-4">
                ...
                <Input id="price" name="price" type="number" step="0.01" placeholder="Unit Price (optional if no quantity)" />
