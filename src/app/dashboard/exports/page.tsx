...
export default function ExportsPage() {
  ...
  async function handleAddExportOrder(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!clientId || !country || !destinationPort || !quantity || !price || !status || !invoiceNumber || !productId) {
    ...
    try {
      ...
      notifyChanged('exports', 'products');
  ...
  const handleEditStatus = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const formData = new FormData(event.currentTarget);
    const newStatus = formData.get('status') as ExportStatus | null;
    const submittedPaymentStatus = formData.get('paymentStatus') as PaymentStatus | null;
    const newPaymentStatus: PaymentStatus = submittedPaymentStatus ?? selectedExport.paymentStatus;
    ...
    if (selectedExport.paymentStatus === 'Paid' && newPaymentStatus !== 'Paid') {
      toast({ variant: 'destructive', title: 'Validation Error', description: 'Paid status cannot be changed back to Pending.' });
      return;
    }
    ...
    try {
      ...
      notifyChanged('exports');
      if (newPaymentStatus === 'Paid' && selectedExport.paymentStatus !== 'Paid') {
        notifyChanged('financial_transactions');
      }
      toast({
          title: "Order Updated",
                ...
                <form onSubmit={handleAddExportOrder}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="paymentStatus" value="Pending" readOnly disabled />
                    </div>
                     <div className="space-y-2">
          ...
          <form onSubmit={handleEditStatus}>
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Select name="paymentStatus" defaultValue={selectedExport?.paymentStatus} required disabled={selectedExport?.paymentStatus === 'Paid'}>
                  <SelectTrigger id="edit-payment-status">
                    <SelectValue placeholder="Select payment status" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="Pending" disabled={selectedExport?.paymentStatus === 'Paid'}>Pending</SelectItem>
